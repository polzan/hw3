...
c) For the antialiasing filter that works 
\begin{figure}[h]
  \centering
...
$M1 = M2 = 3$, $D=1$ and the filter coefficients are the same ones of
Tab.~\ref{tab:dfe_c}.
...
three postcursors of $\eta_n = \psi((n+D)T)$ and its parameters for the state
will be $L_1 = 0$, $L_2 = M_2 = 3$.
...
of transitions to detect the symbol at every sample.
...
\begin{equation}
  \tilde{b}_{K_{in}}(i) = 0 \qquad i = 1,2,\dots64 .
\end{equation}
Then:
\begin{itemize}
\item we compute the transition metrics
...
get the logarith of the state metric
\begin{equation}
  \tilde{v}_k(i) = \tilde{f}_k(i) + \tilde{b}_k(i) \qquad i=1,2,\dots64 ,
\end{equation}
apply the Max-Log-Map approximation to compute the log-likelyhood and
decide on the received symbols
\begin{align}
  \tilde{l}_k(\beta) &= \max_{\substack{i=1,2\dots64 \\ [\vsigma_i]_1 = \beta}} \tilde{v}_k(i) \\
  \hat{a}_k &= \argmax_{\beta \in \mathcal{A}} \tilde{l}_k(\beta)
\end{align}
Like in the Viterbi algorithm the output sequence is shifted of $K_d
-1$ and there is the delay $D$ introdiced by the filter $c$. For the
trellis depth we pick the same value of the Viterbi case $K_d = 30$.

As can also be seen in Fig.~\ref{plot:pe_all}, with a SNR of 11 dB we
get a symbol error probability $P_e = 3.9 \cdot 10^{-4}$.
